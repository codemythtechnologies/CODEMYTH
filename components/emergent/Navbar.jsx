        ...
        {mobile && (
          ...
          >
            <div className="flex h-[84px] items-center justify-between px-6">
              <Logo dark />
              <button onClick={() => setMobile(false)} data-testid="nav-mobile-close" aria-label="Close menu">
                <X className="h-7 w-7" />
              </button>
            </div>
              ...
              {NAV.map((group) => (
                <div key={group.label} className="border-b border-white/10">
                  <button
                    ...
                    className="flex w-full items-center justify-between py-5 font-display text-2xl font-bold tracking-tight"
                  >
                    {group.label}
