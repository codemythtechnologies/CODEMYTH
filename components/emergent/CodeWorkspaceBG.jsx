...
export default function CodeWorkspaceBG() {
  ...
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none absolute inset-0 select-none overflow-hidden bg-[#0d0f13] blur-[3px] opacity-40 scale-105 sm:blur-none sm:opacity-100 sm:scale-100"
    >
      {/* Below the `sm` breakpoint the code window runs full-width
          directly behind the hero headline (it's only docked to a side
          column from `sm` up — see the wrapper below), so on phones it
          reads as messy, half-legible code sitting on top of the copy.
          The typewriter keeps running exactly as before — nothing here
          pauses or disables it — it's just blurred and dimmed into a
          soft ambient backdrop below `sm`, and snaps back to fully sharp
          on larger screens. */}
