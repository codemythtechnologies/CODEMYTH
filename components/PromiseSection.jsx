...
export default function PromiseSection() {
  return (
    <section className="section promise">
      <div className="promise-inner promise-inner-band">
        {PROMISES.map((p, i) => (
          <Reveal className="promise-item promise-item-band" key={p.title} delay={i * 0.05}>
            <div className="promise-band-num">{String(i + 1).padStart(2, "0")}</div>
            <div className="promise-band-body">
              <h4>{p.title}</h4>
              <p>{p.desc}</p>
            </div>
          </Reveal>
