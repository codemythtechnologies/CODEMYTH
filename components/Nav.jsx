...
const SOLUTIONS_LINKS = [
  { id: "business-analysis", title: "Startups & founders", sub: "Idea validation, PRDs and MVP scoping before code" },
  { id: "services", title: "Small & mid-size businesses", sub: "Full-stack apps and landing pages, built and shipped fast" },
  { id: "ai-era", title: "AI-driven teams", sub: "LLM integration, RAG and agent workflows done properly" },
  { id: "approach", title: "Internal products", sub: "Dashboards, admin tools and internal systems" },
];

const COMPANY_LINKS = [
  { id: "ai-era", title: "About", sub: "Why CodeMyth exists and how we work" },
  { id: "team", title: "Team", sub: "The people behind every build" },
  { id: "approach", title: "Approach", sub: "Our five-step delivery process" },
  { id: "contact", title: "Contact", sub: "Start a conversation about your project" },
];
    ...
    <>
      <nav className={`nav${scrolled ? " scrolled" : ""}`}>
        ...
        <div className="nav-center">
          ...
          <div className={`nav-drop${openDrop === "solutions" ? " open" : ""}`} onClick={(e) => e.stopPropagation()}>
            <a
              href="#business-analysis"
              className="nav-drop-trigger"
              onClick={(e) => { e.preventDefault(); go("business-analysis"); }}
              onMouseEnter={() => setOpenDrop("solutions")}
            >
              Solutions
              <svg className="nav-caret" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9" /></svg>
            </a>
            <div className="nav-dropdown" onMouseLeave={() => setOpenDrop(null)}>
              {SOLUTIONS_LINKS.map((s) => (
                <button key={s.id} className="nav-drop-item" onClick={() => { go(s.id); setOpenDrop(null); }}>
                  <b>{s.title}</b><small>{s.sub}</small>
                </button>
              ))}
            </div>
          ...
          <div className={`nav-drop${openDrop === "company" ? " open" : ""}`} onClick={(e) => e.stopPropagation()}>
            <a
              href="#team"
              className="nav-drop-trigger"
              onClick={(e) => { e.preventDefault(); go("team"); }}
              onMouseEnter={() => setOpenDrop("company")}
            >
              Company
              <svg className="nav-caret" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9" /></svg>
            </a>
            <div className="nav-dropdown" onMouseLeave={() => setOpenDrop(null)}>
              {COMPANY_LINKS.map((c) => (
                <button key={c.title} className="nav-drop-item" onClick={() => { go(c.id); setOpenDrop(null); }}>
      ...
      <div className={`nav-mobile-panel${mobileOpen ? " open" : ""}`}>
        ...
        <a onClick={() => go("business-analysis")}>Solutions</a>
        <a onClick={() => go("ai-era")}>AI</a>
        <a onClick={() => go("work")}>Work</a>
        <a onClick={() => go("approach")}>Approach</a>
        <a onClick={() => go("team")}>Company</a>
